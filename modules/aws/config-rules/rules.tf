...
#       policy_text    = <<EOF
# rule check_required_tags when resourceType in [
#   "AWS::IAM::User",
#   "AWS::IAM::Policy",
#   "AWS::IAM::Role",
#   "AWS::Lambda::Function",
...
locals {
  ...
  compliance_resource_types_formatted = join(",\n  ", formatlist("\"%s\"", local.compliance_resource_types))

  tag_rules = join("\n\n", [
    for tag in var.required_tag_keys : <<-EOT
      rule check_required_tag_${replace(tag, "-", "_")} when resourceType in [
        ${local.compliance_resource_types_formatted}
      ] {
        tags["${tag}"] !empty
      }
    EOT
...
resource "aws_config_config_rule" "check_required_tags" {
  ...
  scope {
    compliance_resource_types = local.compliance_resource_types
  }
