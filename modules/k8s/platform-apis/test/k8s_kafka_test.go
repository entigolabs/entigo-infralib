...
func testKafka(t *testing.T, ctx context.Context, cluster, argocd *terrak8s.KubectlOptions) {
	...
	waitSyncedAndReady(t, kfNs, KafkaMSKClusterKind, KafkaClusterName, 120, 30*time.Second)
	if t.Failed() {
		return
	}

	clusterARN := getField(t, kfNs, KafkaMSKClusterKind, KafkaClusterName, ".status.atProvider.arn")
...
// setKafkaApplicationClusterARN patches the ArgoCD application's helm values to include
// the given MSK cluster ARN so that the observer, topic, and user resources are rendered.
...
func cleanupKafka(t *testing.T, cluster, argocd *terrak8s.KubectlOptions) {
	...
	// Trigger MSK cluster and security group deletion (no wait — AWS resource deletion is slow)
	_, _ = terrak8s.RunKubectlAndGetOutputE(t, kfNs, "delete", KafkaMSKClusterKind, KafkaClusterName, "--ignore-not-found")
	_, _ = terrak8s.RunKubectlAndGetOutputE(t, kfNs, "delete", "securitygroup.ec2.aws.upbound.io", KafkaSecurityGroupName, "--ignore-not-found")
