...
func testKafka(t *testing.T, ctx context.Context, cluster, argocd *terrak8s.KubectlOptions) {
	...
	waitSyncedAndReady(t, cluster, KafkaMSKClusterKind, KafkaClusterName, 180, 10*time.Second)
	if t.Failed() {
		return
	}
	clusterARN := getField(t, cluster, KafkaMSKClusterKind, KafkaClusterName, ".status.atProvider.arn")
	require.NotEmpty(t, clusterARN, "MSK cluster ARN should be populated after provisioning")
	patchResource(t, cluster, KafkaMSKKind, KafkaMSKObserverName, `{"spec":{"clusterARN":"`+clusterARN+`"}}`)
...
func cleanupKafka(t *testing.T, cluster, argocd *terrak8s.KubectlOptions) {
	...
	cleanupDeleteAndWait(t, cluster, KafkaMSKClusterKind, KafkaClusterName, 180)
