...
func testTerraformVpcBiz(t *testing.T) {
        options := tf.InitTerraform(t, bucketName, Region, "tf_unit_basic_test_biz.tfvars", map[string]interface{}{})
	testTerraformVpcBizAssert(t, "biz", options)
}

func testTerraformVpcPri(t *testing.T) {
        options := tf.InitTerraform(t, bucketName, Region, "tf_unit_basic_test_pri.tfvars", map[string]interface{}{})
