...
gcloud compute backend-services list --uri | while read line
do
...
gcloud dns record-sets list --zone=gcp-infralib-entigo-io --format="get(name)" | grep -ve "^gcp.infralib.entigo.io.\|^agent.gcp.infralib.entigo.io." | while read -r RECORD_NAME
do
