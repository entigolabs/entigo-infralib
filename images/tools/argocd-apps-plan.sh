...
then
  ...
  repopod=$(kubectl get pod -n $ARGOCD_NAMESPACE \
...
then
  ...
  then
    argocd --server ${ARGOCD_HOSTNAME} --http-retry-max 5 --grpc-web app diff --exit-code=false --app-namespace $app_namespace $app_name
  fi
fi
echo "###############"
