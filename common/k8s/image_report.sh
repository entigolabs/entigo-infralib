...
AWS_BIZ_CONTAINERS=`kubectl get pods -A -o json | jq -r .items[].spec.containers[].image | sort | uniq | sed 's/biz-net-ecr-proxy-/pri-net-ecr-proxy-/g'`
...
security=0
registry=0
for image in $(echo "877483565445.dkr.ecr.eu-north-1.amazonaws.com/biz-net-ecr-proxy-hub/entigolabs/entigo-infralib-testing:latest 877483565445.dkr.ecr.eu-north-1.amazonaws.com/biz-net-ecr-proxy-hub/entigolabs/entigo-infralib-agent:latest $AWS_BIZ_CONTAINERS $AWS_PRI_CONTAINERS $GOOGLE_PRI_CONTAINERS $GOOGLE_BIZ_CONTAINERS" | tr ' ' '\n' | sort | uniq | tr '\n' ' ')
do
  let total++
