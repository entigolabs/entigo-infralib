...
docker run --rm -v "$(pwd)":/project -w /project --entrypoint /bin/bash martivo/kube-score:latest -c "helm lint $VALUES_OPTS --strict ."
if [ $? -ne 0 ]
then
...
docker run --rm -v "$(pwd)":/project -w /project --entrypoint /bin/bash martivo/kube-score:latest -c "helm template $prefix $VALUES_OPTS --skip-tests --namespace $prefix . > /dev/null"
if [ $? -ne 0 ]
then
